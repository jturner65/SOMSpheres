...
public class my3DCanvas {
	...
	public my3DCanvas(SOM_SphereMain _p) {
		p = _p;
	...
	public void buildCanvas(){	
		...
		//p.outStr2Scr("eye in world : " + eyeInWorld.toStrBrf() );
     	//dfCtr = mseLoc;			//will be either on object that can be intersected with or in space, far away, or near object in same depth
     	//if(p.flags[p.debugMode]){
//	     	p.outStr2Scr("Canvas values : canvas ctr : " + scrCtrInWorld.toStrBrf() + " ctr Depth : " + String.format("%.4f",ctrDepth) + "  eyeInWorld : " + eyeInWorld.toStrBrf() + " eyetoctr  :" + eyeToCtr.toStrBrf() + " | rawScrCtrInWorld : " +rawScrCtrInWorld.toStrBrf() );
//	    	p.outStr2Scr("Mouse values : mseLoc : " + mseLoc.toStrBrf()+ " dfctr : " + dfCtr.toStrBrf() + " eyetomse : "+eyeToMse.toStrBrf() + " curDepth : "+String.format("%.4f",curDepth) + " drawSNorm "+drawSNorm.toString() + " ");
     //	}    										
    //	drawCanvas();
	}//buildCanvas()
...
//		p.outStr2Scr("eye in world : " + eyeInWorld.toStrBrf() + " | cur eye pt " + pick(viewDimW/2, viewDimH/2,-.00001f)+ " | up pt " + upInWorld.toStrBrf());
		return p.U(pick(viewDimW/2, viewDimH/2,-.00001f),pick(viewDimW/2, 0,-.00001f));
	}	
	public myVector getUScrRightInWorld(){
//		myPoint rtInWorld = pick(viewDimW, viewDimH/2,-.00001f);
...
class edge{ 
	public SOM_SphereMain p;
	public myPoint a, b;
	public edge (SOM_SphereMain _p){this(_p,new myPoint(0,0,0),new myPoint(0,0,0));}
	public edge (SOM_SphereMain _p, myPoint _a, myPoint _b){p = _p;a=new myPoint(_a); b=new myPoint(_b);}
